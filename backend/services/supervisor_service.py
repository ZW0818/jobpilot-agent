...
from agent.job_classifier_agent import JobClassifierAgent
...
class SupervisorService:
    ...
    def __init__(self) -> None:
        ...
        self.job_classifier_agent = JobClassifierAgent()
    ...
    def run(self, resume_text: str, jd_text: str) -> JobPilotResult:
        ...
        logs.append("JobClassifierAgent 正在识别岗位方向。")
        job_classification = self.job_classifier_agent.run(jd_text)
        logs.append(
            f"已识别岗位方向：{job_classification.domain}，原因：{job_classification.reason}"
        )

        logs.append(f"KnowledgeAgent 正在检索 {job_classification.domain} 方向知识库。")
        rag_context = self.knowledge_agent.run(jd_info, jd_text, job_classification)
        logs.append("KnowledgeAgent 已完成 RAG 知识检索。")

        logs.append("EvaluationAgent 正在计算匹配评分，并归纳优势与缺口。")
        match_result = self.evaluation_agent.run(resume_info, jd_info, rag_context, job_classification)
        logs.append("EvaluationAgent 已完成匹配度评估。")

        logs.append("CoachAgent 正在生成简历优化、面试准备题和 Markdown 报告。")
        rewrite_result, markdown_report = self.coach_agent.run(
            resume_info,
            jd_info,
            match_result,
            rag_context,
